...
def feature_extraction(X):
  ...
  num_of_syllables = 10
  final_data_size = 4*num_of_syllables + 1 + 1 + 1 + 1 + 1 + 1 + 1
      ...
      for day in range(number_of_day):
        ...
        for session in range(number_of_session):
          ...
          # transfer results to mouse_final_data
            ...
            mouse_final_data[row,-8] = recording_split_data["Mother Genotype"].iloc[0]
            ...
            # we already split by mouse and recording, so this should be the same for all recording_split_data. Take the first
            mouse_final_data[row,-7] = recording_split_data["Sex"].iloc[0]
            # PRINT RESULT:
            ...
            # we already split by mouse and recording, so this should be the same for all recording_split_data. Take the first
            mouse_final_data[row,-6] = recording_split_data["ISI_time"].mean()
            # PRINT RESULT:
            # print("for this mouse and this recording, the mean ISI time is:","\n", mouse_final_data[row,-2])

            ## -------------------------------------------
            # calculate feature 8 - age
            ## -------------------------------------------
            print("\n", "--- CALCULATING FEATURE 8: pup age ---", "\n")
            # we already split by mouse and recording, so this should be the same for all recording_split_data. Take the first
            mouse_final_data[row,-5] = recording_split_data["Day"].iloc[0]

            # calculate feature 9 - session
            ## -------------------------------------------
            print("\n", "--- CALCULATING FEATURE 9: session ---", "\n")
            # we already split by mouse and recording, so this should be the same for all recording_split_data. Take the first
            mouse_final_data[row,-4] = recording_split_data["Session"].iloc[0]
            ## -------------------------------------------
            # calculate feature 10 - strain
            ## -------------------------------------------
            print("\n", "--- CALCULATING FEATURE 10: pup strain ---", "\n")
            # we already split by mouse and recording, so this should be the same for all recording_split_data. Take the first
            mouse_final_data[row,-3] = recording_split_data["Strain"].iloc[0]
